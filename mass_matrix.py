...
def A_local(e):
    ...
    Dp = np.abs(x.diff("xi")*y.diff("eta")
        ...
        for j in range(4):
            ...
            for c_x in range(len(weights)):
                for c_y in range(len(weights)):
                    x_cx = x.replace("xi", points[c_x]).replace("eta", points[c_y])
                    x_cy = x.replace("xi", points[c_x]).replace("eta", points[c_y])

                    A_e[i,j] += weights[c_x]*weights[c_y]*Dp*(
